...
*   POC of a EndScene and Reset Hook inside MW3
*/
...
HWND hwnd;
void InitDX(IDirect3DDevice9* device_) {
    device = device_;
...
#define TableIndex 119
DWORD vTableAddr = NULL;
void* Vtable[TableIndex];
void HackThread(HMODULE mod) {
    ...
    #define EndSceneTable Vtable[EndSceneIndex]

    #define ResetIndex 16
    #define ResetTable Vtable[ResetIndex]

    std::cout << "Vtable addr: " << std::hex << vTableAddr << "\n";
    memcpy((void**)Vtable, *(void***)vTableAddr, sizeof(Vtable));
    std::cout << "endScene addr: " << std::hex << EndSceneTable << "\n";
    std::cout << "reset addr: " << std::hex << ResetTable << "\n";

    og_endScene = (typedef_endScene)EndSceneTable;
    ...
    og_reset = (typedef_reset)ResetTable;
    if (!FAILED(DetourTransactionBegin()))
    ...
    og_wndProc = (WNDPROC)SetWindowLongW(hwnd, -4, (LONG)wndProc);
